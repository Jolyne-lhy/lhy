...
from ep_utils.helpers import list_of_dict_search
...
def get_nx_graph(wds, weight_mode='unweighted', get_head=False):
    ...
    # Modification: A weighted pipe length graph
    elif weight_mode == 'weighted_pipe_length':
        max_weight = 0
        for pipe in wds.pipes:
            if (pipe.from_node.index in junc_dict) and (pipe.to_node.index in junc_dict):
                weight  = pipe.length / ((pipe.diameter*0.001)**5)
                G.add_edge(pipe.from_node.index, pipe.to_node.index, weight=float(weight), name=str(pipe).replace("<epynet.Pipe with id '","").replace("'>", ""))
                if weight > max_weight:
                    max_weight = weight
        for (_,_,d) in G.edges(data=True):
    ...
    # Modification: A real pipe length graph
    elif weight_mode == 'real_pipe_length':
        for pipe in wds.pipes:
            if (pipe.from_node.index in junc_dict) and (pipe.to_node.index in junc_dict):
                weight  = pipe.length
                G.add_edge(pipe.from_node.index, pipe.to_node.index, weight=float(weight), name=str(pipe).replace("<epynet.Pipe with id '","").replace("'>", "")) 
